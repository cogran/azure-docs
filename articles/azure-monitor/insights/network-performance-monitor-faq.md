...
The capability to monitor networks using Linux-based nodes is now in generally available. Acccess the agent [here](../../virtual-machines/extensions/oms-linux.md). Linux agents provide monitoring capability only for NPM's Performance Monitor capability, and are not available for the Service Connectivity Monitor and ExpressRoute Monitor capabilities
